...
KissSlider.prototype.moveTo = function(index, dir, noAnim) {
	...
	if(noAnim || this.options.noAnim) {
		...
		$target.css('left', targetLeftFrom).add($current).animate({
			left: '+=' + distance
...
KissSlider.actions = {
	next: function(instance, actionParams) {
		instance.nextSlide();
	},
	previous: function(instance, actionParams) {
		instance.prevSlide();
	},
	moveTo: function(instance, actionParams) {
		instance.moveTo(actionParams.index, actionParams.dir);
	}
};
